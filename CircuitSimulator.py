...
class QCircuit(QuantumCircuit):
    ...
    def colormap(self, density):
        ...
        z = np.array([[1, 0], [0, -1]])
        avgZ = np.trace(np.matmul(density, z))
        l = 100 * (1 - avgZ) * 0.5

        x = np.array([[0, 1], [1, 0]])
        avgx = np.trace(np.matmul(density, x))
        a = 127 * avgx

        y = np.array([[0, -1j], [1j, 0]])
        avgy = np.trace(np.matmul(density, y))
        b = 127 * avgy
    ...
    def gate_map(self):
        ...
        for gate, qubits, _ in self.data:
            qubits = [q.index for q in qubits] # redefine qubits to be the indicies
    ...
    def draw_color(self, return_fig=False):
        ...
        for i in range(numpositions):
            ...
            density = self.getdensitymatrix(tempcirc)
